...
class JointLink():
    ...
    # X_r_to includes joint movement
    # X_r_to = XJn * XJ(n-1) * ... J1 * XT0 * XJ0 * X0
    ...
    def joint_position(self):
        assert self.X_r_to is not None, "link tee is not built yet"
        _, _, x, y = Xtoscxy(self.X_r_to)
        return Matrix([x, y])

    def tool_position(self):
        assert self.X_r_to is not None, "link tee is not built yet"
        _, _, x, y = Xtoscxy(self.Xnext())
        return Matrix([x, y])

    #px py are abs coordinate
    def apply_force(self, fx, fy, px, py, fa=0):
        self.fa = px * fy  - py * fx + fa
        self.fy = fy 
        self.fx = fx 
...
class StickJointLink(JointLink):
    ...
    def draw_cmds(self):
        return draw_lineseg_turtle_cmd(self.X_r_to, self.l, name=self.name)
...
class WheelJointLink(JointLink):
    ...
    def draw_cmds(self):
        return draw_dir_circle_cmd(self.X_r_to, self.r, name=self.name)
